...
class LongitudinalMpc:
  ...
  def process_lead(self, lead, trafficModeActive):
    v_ego = self.x0[1]
    if lead is not None and lead.status:
      x_lead = lead.dRel - (self.increased_stopping_distance if not trafficModeActive else 0)
      v_lead = lead.vLead
      a_lead = lead.aLeadK
  ...
  def update(self, lead_one, lead_two, v_cruise, x, v, a, j, t_follow, trafficModeActive, personality=log.LongitudinalPersonality.standard):
    v_ego = self.x0[1]
    self.status = lead_one.status or lead_two.status

    lead_xv_0 = self.process_lead(lead_one, trafficModeActive)
    lead_xv_1 = self.process_lead(lead_two, trafficModeActive)
